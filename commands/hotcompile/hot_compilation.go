...
func HotCompilationRun() {
	var paths []string
	ReadAppDirectories(console.RootPath, &paths)
	for _, path := range console.HotCompileCfg.WatchPaths {
		ReadAppDirectories(path, &paths)
	}
	files := []string{}
	if console.BuildPkg == "" {
		console.BuildPkg = console.HotCompileCfg.BuildPkg
	}
	if console.BuildPkg != "" {
		files = strings.Split(console.BuildPkg, ",")
	}
...
func Autobuild(files []string) {
	...
	for _, prevCmd := range console.HotCompileCfg.PrevBuildCmds {
		log.Infof("Run external cmd '%s'", prevCmd)
		cmdArr := strings.Split(prevCmd, " ")
		prevCmdExec := exec.Command(cmdArr[0])
		prevCmdExec.Env = append(os.Environ(), console.HotCompileCfg.Envs...)
	...
	args := []string{"build"}
	args = append(args, "-o", console.HotCompileCfg.OutputAppPath)
	args = append(args, console.HotCompileCfg.BuildArgs...)
	if console.HotCompileCfg.BuildTags != "" {
		args = append(args, "-tags", console.HotCompileCfg.BuildTags)
	}
	...
	if !console.HotCompileCfg.DisableRun {
		if len(console.HotCompileCfg.RunCmd) != 0 {
			Restart(console.HotCompileCfg.RunCmd, true)
		} else {
			Restart(console.HotCompileCfg.OutputAppPath, false)
		}
	}
}
...
func Start(appname string, isCmd bool) {
	...
	cmd.Args = append([]string{appname}, console.HotCompileCfg.CmdArgs...)
	cmd.Env = append(os.Environ(), console.HotCompileCfg.Envs...)
...
func checkIfWatchExt(name string) bool {
	for _, s := range console.HotCompileCfg.WatchExts {
...
func ReadAppDirectories(directory string, paths *[]string) {
	...
	for _, fileInfo := range fileInfos {
		...
		if !console.HotCompileCfg.VendorWatch && strings.HasSuffix(fileInfo.Name(), "vendor") {
			continue
		}
...
func isExcluded(filePath string) bool {
	for _, p := range console.HotCompileCfg.ExcludedDirs {
		absP, err := path.Abs(p)
		if err != nil {
