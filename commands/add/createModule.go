...
import (
	"bufio"
	"bytes"
	"cvgo/commands/add/gencvgo"
	"cvgo/commands/add/genfiber"
	"cvgo/console"
	"errors"
	"fmt"
...
func createCodeFile(modName, webserver string, supportSwagger bool) {
	...
	if webserver == "" {
		...
		if webserver == "cvgo" {
			gencvgo.CreateWebserverWithCvgo(moduleDir, modName)
		} else if webserver == "fiber" {
			genfiber.CreateWebserver(moduleDir, modName, supportSwagger)
		}
	}
